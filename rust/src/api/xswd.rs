...
use dapa_common::tokio::spawn_task;
pub use dapa_common::tokio::sync::mpsc::UnboundedReceiver;
pub use dapa_common::tokio::sync::oneshot::Sender;
pub use dapa_wallet::api::AppState;
use dapa_wallet::api::{APIServer, Permission, PermissionResult};
pub use dapa_wallet::wallet::XSWDEvent;
...
impl XSWD for DapaWallet {
    ...
    async fn get_application_permissions(&self) -> Result<Vec<AppInfo>> {
        let lock = self.get_wallet().get_api_server().lock().await;
        let api_server: &dapa_wallet::api::APIServer<
            std::sync::Arc<dapa_wallet::wallet::Wallet>,
        > = lock
            .as_ref()
