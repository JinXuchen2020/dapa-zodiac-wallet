...
use serde_json::json;
use dapa_common::api::{DataElement, DataValue};
use dapa_common::config::{COIN_DECIMALS, DAPA_ASSET};
use dapa_common::crypto::{Address, Hash, Hashable, Signature};
use dapa_common::network::Network;
use dapa_common::serializer::Serializer;
use dapa_common::transaction::builder::{
    FeeBuilder, MultiSigBuilder, TransactionTypeBuilder, TransferBuilder, UnsignedTransaction,
};
use dapa_common::transaction::multisig::{MultiSig, SignatureId};
use dapa_common::transaction::BurnPayload;
pub use dapa_common::transaction::Transaction;
use dapa_common::utils::{format_coin, format_dapa};
use dapa_wallet::precomputed_tables;
pub use dapa_wallet::transaction_builder::TransactionBuilderState;
use dapa_wallet::wallet::{RecoverOption, Wallet};
...
pub async fn create_dapa_wallet(
    name: String,
    password: String,
    network: Network,
    seed: Option<String>,
...
impl DapaWallet {
    ...
    pub async fn update_precomputed_tables(
        ...
        let mut existing_tables_lock = (&*self.wallet)
            .get_precomputed_tables()
            .write()
            .expect("Failed to lock existing precomputed tables");
    ...
    pub async fn get_dapa_balance(&self) -> Result<String> {
        let storage = self.wallet.get_storage().read().await;
        let balance = storage
            .get_plaintext_balance_for(&DAPA_ASSET)
            .await
            .unwrap_or(0);
        Ok(format_dapa(balance))
    }
    ...
    pub async fn get_known_assets(&self) -> Result<HashMap<String, String>> {
        ...
        let assets = storage
            ...
            .await?
            .into_iter()
            .filter_map(|(hash, asset_data)| {
                Some((hash.to_hex(), json!(asset_data).to_string()))
            })
    ...
    pub async fn track_asset(&self, asset: String) -> Result<bool> {
        Hash::from_hex(&asset).context("Invalid asset")?;
        // let storage = self.wallet.get_storage().read().await;
        let result = false;
    ...
    pub async fn untrack_asset(&self, asset: String) -> Result<bool> {
        Hash::from_hex(&asset).context("Invalid asset")?;
        // let storage = self.wallet.get_storage().read().await;
        // if storage.contains_asset(&asset_hash).await.unwrap_or(false) {
        //     storage.remove_asset(&asset_hash).await?;
        //     info!("Asset {} untracked", asset);
        //     let result = true;
        //     Ok(result)
        // } else {
        //     info!("Asset {} not found in storage", asset);
