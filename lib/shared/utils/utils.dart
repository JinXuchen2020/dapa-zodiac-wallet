...
import 'package:zodiac/features/wallet/domain/destination_address.dart';
...
String formatCoin(int value, int decimals, String ticker) {
  ...
  final dapaValue = value / pow(10, decimals);
  return formatter.format(dapaValue);
}

String formatDapa(int value, rust.Network network) {
  final formatter = NumberFormat.currency(
    locale: 'fr_FR',
    decimalDigits: AppResources.dapaDecimals,
    symbol: getDapaTicker(network),
  );
  final dapaValue = value / pow(10, AppResources.dapaDecimals);
  return formatter.format(dapaValue);
}

String getDapaTicker(rust.Network network) {
  switch (network) {
